...
ssize_t dsi_panel_set_disp_param(struct drm_connector* connector, u32 cmd)
{
	...
	struct fb_drm_notify_data g_notify_data;
	struct fb_drm_notify_data g_notify_data1;
	int event = FB_DRM_BLANK_POWERDOWN;
	int event1 = FB_DRM_BLANK_UNBLANK;
	...
	switch (cmd_temp) {
		...
		{
			if (!(panel_ext && panel_ext->funcs &&
			      panel_ext->funcs->hbm_control))
				break;
			dsi->normal_hbm_flag = true;
			panel_ext->funcs->hbm_control(dsi->panel, true);
			break;
		}
		case DISPPARAM_HBM_OFF:
		{
			...
			panel_ext->funcs->hbm_control(dsi->panel, false);
			break;
		}
		default:
			break;
				  ...
				  const void *data, size_t len, unsigned int index,unsigned int dst_mode)
{
	...
	/* set BL cmd */
		if (index > 1)
       	 	{
          		DDPINFO("%s add fps mtk_dsi_vm_cmdq\n", __func__);
			mtk_dsi_vm_cmdq(dsi, &msg, handle);
        	}
		else
        	{
                        DDPINFO("%s add fps mtk_dsi_vm_cmdq_vbp\n", __func__);
			mtk_dsi_vm_cmdq_vbp(dsi, &msg, handle);
        	}
	}
        else
	{
                DDPINFO("%s add fps mtk_dsi_vm_cmdq2\n", __func__);
    		mtk_dsi_vm_cmdq(dsi, &msg, handle);
	}
	/* clear VM_CMD_DONE */
	cmdq_pkt_write(handle, dsi->ddp_comp.cmdq_base,
	...
	struct mtk_drm_crtc *mtk_crtc, struct drm_crtc_state *old_state)
{
	...
	if(1){
		 ...
		 * because fps change need stop & re-start vdo mode
		 */
          	DDPINFO("%s add fps adjusted_mode.vrefresh == 90\n", __func__);
		if (dsi->mipi_hopping_sta) {
			DDPINFO("%s,mipi_clk_change_sta\n", __func__);
...
#else
                DDPINFO("%s add fps adjusted_mode.vrefresh == 1\n", __func__);
#ifdef CONFIG_MTK_MT6382_BDG
		...
		mtk_dsi_porch_setting(comp, handle, DSI_VFP, vfp);
                ...
                DDPINFO("%s add fps adjusted_mode.vrefresh ==  my else\n", __func__);
            mtk_dsi_send_switch_cmd_vfp(dsi, handle, mtk_crtc, src_mode, adjusted_mode.vrefresh); //下切换命令
            			cmdq_pkt_wfe(handle,
				 mtk_crtc->gce_obj.event[EVENT_VDO_EOF]);                         //等eof
			cmdq_pkt_wfe(handle,
					mtk_crtc->gce_obj.event[EVENT_DSI0_SOF]);                     //等dsi sof
			cmdq_pkt_wfe(handle,
				 mtk_crtc->gce_obj.event[EVENT_VDO_EOF]);                        //多等一帧
			cmdq_pkt_wfe(handle,
					mtk_crtc->gce_obj.event[EVENT_DSI0_SOF]);                     //等dsi sof
			cmdq_pkt_wfe(handle,
				 mtk_crtc->gce_obj.event[EVENT_VDO_EOF]);                        //多等二帧
            
            		if (dsi->mipi_hopping_sta) {                                   //mipi hopping
				DDPINFO("%s,mipi_clk_change_sta\n", __func__);
				vfp = dsi->ext->params->dyn.vfp;
			} else                                                             
				vfp = adjusted_mode.vsync_start -
					adjusted_mode.vdisplay;
			dsi->vm.vfront_porch = vfp;
			mtk_dsi_porch_setting(comp, handle, DSI_VFP, vfp);                //设置对应帧率的vfp
          }
